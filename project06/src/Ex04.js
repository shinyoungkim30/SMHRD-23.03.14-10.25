...
const Ex04 = () => {
    ...
    useEffect(()=>{
        ...
        // case 1) Fetch API
        fetch(movieUrl)
        .then(res => res.json()) // 받아온 데이터를 json형식의 데이터로 반환
        .then(res => {
            // console.log('fetch res :', res)
            // Q. fetch API로 가지고 온 값을 1~10위 까지 화면에 띄워주자
            // 순위. 영화제목 - 개봉일자
            // console.log(res.boxOfficeResult.dailyBoxOfficeList[0]);
  ...
  return (
    <div className='container'>
        <h1>영화 순위</h1>
        {list.map(item => <p key={item.rank}>{item.rank}. {item.movieNm} - {item.openDt}</p>)}
    </div>
  )
}
